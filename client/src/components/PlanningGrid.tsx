...
const STATUS_OPTIONS = [
  ...
  { value: "EN_SERVICE", label: "En service", color: "bg-green-100 text-green-800" },
  { value: "EN_SERVICE_DEBRIEF", label: "Debrief", color: "bg-yellow-100 text-yellow-800" },
  { value: "INDISPONIBLE", label: "Indispo", color: "bg-red-100 text-red-800" },
  { value: "REMPLACANT", label: "Remplac.", color: "bg-blue-100 text-blue-800" },
];
...
export default function PlanningGrid({
  ...
  const fetchPlanning = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(
        `/events/${eventId}/departments/${departmentId}/planning`
      );
  ...
  const handleStatusChange = (memberId: number, status: string | null) => {
    // Enforce: only one EN_SERVICE_DEBRIEF per department
    const updated = members.map((m) => {
      if (m.id === memberId) {
        return { ...m, status };
      }
      if (status === "EN_SERVICE_DEBRIEF" && m.status === "EN_SERVICE_DEBRIEF") {
  ...
  if (loading) {
    return <div className="p-4 text-gray-500">Chargement du planning...</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-4 text-sm">
        ...
        {dirty && !saving && (
          <span className="text-orange-500">Modifications non sauvegardees</span>
        )}
        {!dirty && !saving && (
                      ...
                      <button
                        ...
                        className={`px-2 py-1 text-xs rounded-md transition-colors ${
                          member.status === option.value
                            ? option.color + " font-semibold ring-2 ring-offset-1 ring-blue-400"
